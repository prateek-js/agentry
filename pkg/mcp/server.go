...
AUTOMATION — when the user wants a scheduled job (cron, "every morning", "hourly", "every 5 minutes") or a webhook handler (Stripe/GitHub/Slack events), call ` + "`project_create kind=automation`" + ` — do NOT use ` + "`python-script`" + ` and do NOT hand-roll a polling loop, a cron container, or a custom logs page. That kind copies the ` + "`@agentry/automation`" + ` template; then ` + "`npm install`" + `, read ` + "`docs_read(\"skills/automation\")`" + `, write schedules in ` + "`automations/jobs.ts`" + ` and webhooks in ` + "`automations/hooks.ts`" + `, and ` + "`project_start`" + `. Run history, payloads, Run-now and Replay come free at ` + "`/_agentry`" + `, which shows the LIVE storage backend. History persists to a bound DB (postgres/mysql/mongo/redis); in-memory only when none is bound. The backend is chosen at process start, so after ` + "`agentry service bind postgres`" + ` you MUST restart the automation (project_stop + project_start) for it to switch — then read the actual backend from ` + "`/_agentry`" + ` and report THAT; never assume "ephemeral".
